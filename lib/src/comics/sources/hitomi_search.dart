// Search index algorithm adapted from PicaComic (MIT, Copyright 2023 Nyne).
import 'dart:convert';
...
class HitomiSearch {
  ...
  HitomiSearch(this.keyword, this.baseDomain, this.dio);
  ...
  Future<List<int>> search() async {
    ...
    final terms = keyword
        ...
        .toList();
    final positive = terms.where((t) => !t.startsWith('-')).toList();
    final negative = terms.where((t) => t.startsWith('-')).toList();
    var ids = positive.isEmpty
        ? await getGalleryIdsFromNozomi(null, 'index', 'all')
        : await getGalleryIdsForQuery(positive.removeAt(0));
    for (final term in positive) {
      final matches = (await getGalleryIdsForQuery(term)).toSet();
      ids = ids.where(matches.contains).toList();
    }
    for (final term in negative) {
      final matches = (await getGalleryIdsForQuery(term.substring(1))).toSet();
      ids = ids.where((id) => !matches.contains(id)).toList();
    }
    return ids;
  }
  ...
  Future<List<int>> getGalleryIdsFromNozomi(
    ...
    var bytes = (await dio.get<Uint8List>(
      url,
  ...
  Future<Uint8List?> getUrlAtRange(String url, List<int> range) async {
    ...
    var res = await dio.get<List<int>>(
      url,
      options: Options(
