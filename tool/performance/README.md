...
- 使用固定本地封面的真实 Mini Player → Player Cover Page 路由；分别执行 5 轮点击开关和 5 轮交互式开关，交互场景包含取消与回弹。
...
- 首页帧 p95 至少改善 25% 或达到 16.7ms；已达预算的基线最多回退 5%。
- 自动与交互式播放器路由帧 p95 都必须至少改善 25%，每个场景至少采集 200 帧。
