...
void main() {
  group('AudioFileUrlResolver', () {
    ...
    test(
      'online resolver handles relative media stream URLs and tokens',
      () async {
        final resolver = AudioFileUrlResolver(
          pathContext: _posixPathContext,
          resolveDownloadedPath: (_, __) async => null,
    ...
    test(
      'online resolver falls back to media stream endpoint by hash',
      () async {
        final resolver = AudioFileUrlResolver(
          pathContext: _posixPathContext,
          resolveDownloadedPath: (_, __) async => null,
    ...
    test(
      'offline resolver returns local file URL only when file exists',
      () async {
        final resolver = AudioFileUrlResolver(
          pathContext: _posixPathContext,
          resolveDownloadedPath: (_, __) async => null,
    ...
    test(
      'offline resolver prefers localRelativePath for sanitized downloads',
      () async {
        final resolver = AudioFileUrlResolver(
          pathContext: _posixPathContext,
          resolveDownloadedPath: (_, __) async => null,
        ...
        expect(
          target.requireTarget.localPath,
          '/downloads/123/Disc_1/track_.mp3',
        );
    ...
    test(
      'offline resolver prefers explicit localPath from disk scanner',
      () async {
        final resolver = AudioFileUrlResolver(
          pathContext: _posixPathContext,
          resolveDownloadedPath: (_, __) async => null,
    ...
    test('offline playback target includes paths and local artwork', () async {
      ...
      expect(
        result.requireTarget.localPath,
        '/downloads/123/Disc 1/track01.mp3',
      );
      expect(
        result.requireTarget.artworkUrl,
        'file:///downloads/123/cover.jpg',
      );
    });

    test(
      'offline playback target can use an imported RJ work directory',
      () async {
